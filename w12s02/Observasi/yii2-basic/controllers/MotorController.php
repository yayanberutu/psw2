...
class MotorController extends Controller
{
    ...
    public function actionActiveRecordMotor(){
        $motor = new Motor();
        $motor->id = 1;
        $motor->name = "Yamaha";
        $motor->price = 12000;
    ...
    public function actionActiveRecordMotorUpdate(){
        $motor = Motor::findOne(1);
        $motor->name = "XYZ";
        $motor->save();
    } 

    public function actionContohLazyLoading(){
        $motors = Motor::find()->all;
        foreach ($motors as $motor){
            echo "Motor : " . $motor->name . "<br/>";
            foreach ($motor->orders as $order) {
